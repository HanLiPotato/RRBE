...
def embedB(imgB, dataList, Aindex):
    ...
    matrix_input = matrix_input.astype(int)
    embedRound=1
    matrix_output ,nextInfoList= encode.encode(matrix_input, dataList, Aindex,embedRound)
    while len(nextInfoList) !=0:
        embedRound +=1
        print('embedRound: ',embedRound)
        matrix_output,nextInfoList=encode.encode(matrix_output,nextInfoList,Aindex,embedRound)
