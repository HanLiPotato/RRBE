...
class LoginDialog(QDialog,Ui_LoginDialog):
    def __init__(self,parent=None):
        ...
        self.visitorRadio.clicked.connect(self.enableEdit)
    ...
    def checkNameEdit(self):
        ...
        if not self.nameEdit.text() and not self.passwdEdit.text():
            self.buttonBox.button(QDialogButtonBox.Ok).setEnabled(False)
        else:
            self.buttonBox.button(QDialogButtonBox.Ok).setEnabled(True)
...
def loginDialog(parent=None):
    ...
    if not login.isAccepted:
        ...
        return (str(login.nameEdit.text()), str(login.passwdEdit.text()), False)
...
class InputMessage(QMainWindow):
    show_sig = PyQt5.QtCore.pyqtSignal(int)
    ...
    def show_window(self, show_):
        if show_:
            self.message = QInputDialog.getText(self,'encry','key',QLineEdit.Normal,'')
        else:
            self.message = ''
