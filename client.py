...
class MyMainGui(QWidget,Ui_Form):
    def __init__(self,parent=None):
        ...
        #self.Local_Upload.clicked.connect(self.upload)
        ...
        self.inputMessage = InputMessage(self)
    ...
    def local_right_menu(self, pos):
        ...
        # 解密图像
            ...
            try:
                ...
                decrypt = QInputDialog.getText(self, '解密图像', '请输入解密密钥', QLineEdit.Normal)
                if not decrypt[1]:
                    return
        ...
        # 提取信息
            ...
            try:
                ...
                extract = QInputDialog.getText(self, '提取信息', '请输入数据密钥', QLineEdit.Normal)
                if not extract[1]:
                    return
    ...
    def initialize(self):
        ...
        self.local_pwd = os.getenv('HOME') if os.name == 'posix' else 'C:\\Users\\10712\\Desktop'
    ...
    def login(self):
        ...
        if ask[-1] == 1:
            resp = self.ftp.sendcmd('regi ' + user + ' ' + passwd)
            self.login_again()
    ...
    def addItemToRemoteFileList(self, content):
        mode, num, size, date, filename = self.parseFileInfo(content)
    ...
    def upload(self):
        ...
        try:
            ...
            fp.set_pasv(0)
            fp.cwd(self.pwd)
            resp = fp.storbinary(cmd='STOR '+dstfile, fp=file, callback=callback)
            print(resp)
            file.close()
